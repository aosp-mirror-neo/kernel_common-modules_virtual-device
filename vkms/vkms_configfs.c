					       ...
					       const char *page, size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		...
		if (plane->dev->enabled || (val & vkms_config_plane_get_default_rotation(plane->config)) != vkms_config_plane_get_default_rotation(plane->config))
			return -EINVAL;
				       ...
				       const char *page, size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		/* Ensures that the default rotation is included in supported rotation */
		if (plane->dev->enabled || (val & vkms_config_plane_get_default_color_range(plane->config)) !=
					       ...
					       const char *page, size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		/* Ensures that the default rotation is included in supported rotation */
		if (plane->dev->enabled || (val & vkms_config_plane_get_supported_color_range(plane->config)) != val)
			return -EINVAL;
						    ...
						    const char *page, size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		/* Ensures that the default rotation is included in supported rotation */
		if (plane->dev->enabled || (val & vkms_config_plane_get_default_color_encoding(plane->config)) !=
						  ...
						  const char *page, size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		/* Ensures that the default rotation is included in supported rotation */
		if (plane->dev->enabled || (val & vkms_config_plane_get_supported_color_encoding(plane->config)) != val)
			return -EINVAL;
		...
		while (ptr < count) {
			if (page[ptr] == '+') {
				...
				if (tmp[0] == '*') {
					...
					ret = vkms_config_plane_add_format(plane->config, *(int *)tmp);
					if (ret)
						return ret;
					ptr += 4;
				}
	...
	scoped_guard(mutex, &connector->dev->lock)
	{
		if (!connector->dev->enabled) {
			...
			if (!was_enabled && enabled) {
				...
				connector_cfg->connector = vkms_connector_hot_add(connector->dev->config->dev, connector_cfg);
...
static void connector_release(struct config_item *item)
{
	...
	scoped_guard(mutex, lock) {
		/*
		 * This is not coherent: if you remove a "static" connector folder at runtime, it will not disapear
		 * But if you remove a "dynamic" connector folder at runtime, it will be removed from drm
		 * Is it an issue ?
		 */
		if (connector->dev->enabled && connector_cfg->dynamic && connector_cfg->enabled)
			vkms_connector_hot_remove(connector->dev->config->dev, connector_cfg->connector);
						  ...
						  struct config_item *target)
{
	...
	scoped_guard(mutex, &connector->dev->lock) {
		if (connector->dev->enabled && connector_cfg->enabled) {
			if (!connector_cfg->dynamic)
				return -EBUSY;
			ret = vkms_connector_hot_attach_encoder(
				connector->dev->config->dev,
