...
enum dxgkvmb_commandtype_global {
	...
	DXGK_VMBCOMMAND_CREATECOMPOSITIONTARGETS = 3001,
...
enum dxgkvmb_commandtype_host_to_vm {
	...
	DXGK_VMBCOMMAND_SIGNALDISPLAYCHANGE,
...
struct dxgkvmb_command_getallocationsize {
	...
	struct d3dkmthandle		device;
	u32				alloc_count;
/* u32 priv_drv_data_size for each alloc_count */
/* u8 priv_drv_data[] for each alloc_count */
};

struct dxgkvmb_command_getallocationsize_return {
	struct ntstatus			status;
	u32				alloc_count;
	/* u64 size for each alloc_count */
};
...
struct dxgkvmb_command_presentvirtual2 {
	struct dxgkvmb_command_vm_to_host hdr;
	u64 acquire_layers_semaphore_nthandle; // wait semaphore for acquisition of
		// layers
...
struct dxgkvmb_command_createcompositiontargets {
	struct dxgkvmb_command_vm_to_host hdr;
	u32 target_memory_nthandle_count; // Layers fd count
	u64 private_data_size; // Nexus private data size
	// u64 target_memory_nthandle (s)
	// u8 private_data
};
