...
int dxgvmb_send_present_virtual2(struct dxgprocess *process,
				 ...
				 __u64 acquire_layers_semaphore_nthandle,
				 __u64 acquire_target_semaphore_nthandle,
				 ...
				 u64 *layer_memory_nthandle)
{
	...
	u32 cmd_size = sizeof(struct dxgkvmb_command_presentvirtual2) +
		       layers_fd_size + args->private_data_size;
	u8 *write_pos;
	ret = init_message(&msg, NULL, process, cmd_size);
	if (ret)
		return ret;
	...
	command_vm_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_PRESENTVIRTUAL2,
				 process->host_handle);
	command->acquire_layers_semaphore_nthandle =
		acquire_layers_semaphore_nthandle;
	command->acquire_target_semaphore_nthandle =
		acquire_target_semaphore_nthandle;
	command->release_layers_semaphore_nthandle =
		release_layers_semaphore_nthandle;
	command->release_target_semaphore_nthandle =
